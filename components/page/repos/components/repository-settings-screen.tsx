...
export function RepositorySettingsPage({
  ...
  const supportedEvents = notificationsData?.supportedEvents ?? [
    'issues',
    'pull_request',
    'push',
    'issue_comment',
      ...
      <Card className="border-white/70 bg-white/80 shadow-lg shadow-slate-200/40 backdrop-blur dark:border-white/10 dark:bg-slate-950/70 dark:shadow-none">
        <CardHeader className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <CardDescription>
              Route real-time GitHub events from this repository directly to
              your integrated bots. SyncHub will automatically provision
              webhooks on your behalf to forward live events.
            </CardDescription>
          </div>
        </CardHeader>
        <CardContent>
          <div className="space-y-6">
            {isLoadingNotifications ? (
              <div className="text-muted-foreground rounded-3xl border border-dashed border-slate-300 px-5 py-8 text-sm dark:border-slate-700">
                <Spinner className="mr-2 inline size-4" /> Loading your
                notification rules...
              </div>
            ) : (
              <div className="space-y-6">
                  ...
                  <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                    <div>
                      ...
                      <p className="mt-2 text-sm text-slate-600 dark:text-slate-300">
                        SyncHub provisions a single repository webhook and
                        routes live GitHub events to your connected channels
                        from there.
                      </p>
                    </div>
                ...
                <div className="grid gap-6 md:grid-cols-2">
                  ...
                  <div className="rounded-3xl border border-blue-200/50 bg-blue-50/30 px-6 py-8 dark:border-blue-900/40 dark:bg-blue-900/10">
                    ...
                    <div className="space-y-4">
                      {notificationEvents.map((eventType) => {
                        const rule = notificationsData?.rules.find(
                          (r) => r.provider === 'TELEGRAM'
                        )
                        const isEnabled =
                          rule?.events.includes(eventType) ?? false

                        return (
                          <label
                            key={`tg-${eventType}`}
                            className="glass-panel border-border/50 hover:bg-primary/5 flex cursor-pointer items-start gap-4 p-4 shadow-sm transition-all duration-300"
                          >
                            <input
                              type="checkbox"
                              checked={isEnabled}
                              ...
                              onChange={(e) => {
                                ...
                                updateRule.mutate(
                                  ...
                                  },
                                  {
                                    onError: (err) => {
                                      toast.error(
                                        err.message || 'Failed to update rule'
                                      )
                                    },
                                  }
                                )
                            ...
                            />
                            <div>
                              <span className="block text-sm font-medium text-slate-800 capitalize dark:text-slate-200">
                                {eventType === 'push'
                                  ? 'Commits (Pushes)'
                                  : eventType.replace('_', ' ')}
                              </span>
                              <span className="mt-0.5 block text-xs text-slate-500 dark:text-slate-400">
                                {eventType === 'issues' &&
                                  'Alert when new issues are opened.'}
                                {eventType === 'pull_request' &&
                                  'Alert when code is submitted for review.'}
                                {eventType === 'push' &&
                                  'Alert when new code is pushed to this repo.'}
                                {eventType === 'issue_comment' &&
                                  'Alert when someone adds a new issue or PR comment.'}
                              </span>
                            </div>
                          </label>
                        )
                  ...
                  <div className="rounded-3xl border border-indigo-200/50 bg-indigo-50/30 px-6 py-8 dark:border-indigo-900/40 dark:bg-indigo-900/10">
                    ...
                    <div className="space-y-4">
                      {notificationEvents.map((eventType) => {
                        const isEnabled =
                          discordRule?.events.includes(eventType) ?? false
                        ...
                        const selectedChannelId =
                          currentOverrides?.[eventType] ?? ''

                        return (
                          <div
                            key={`dc-${eventType}`}
                            className="glass-panel border-border/50 hover:bg-primary/5 space-y-3 p-4 shadow-sm transition-all duration-300"
                          >
                            ...
                            <div className="space-y-2">
                              <label className="text-muted-foreground block text-xs font-medium tracking-[0.16em] uppercase">
                                Discord channel
                              </label>
                              <select
                                ...
                                className="w-full rounded-2xl border border-slate-200 bg-white px-3 py-2 text-sm transition outline-none focus:border-indigo-300 focus:ring-2 focus:ring-indigo-200 dark:border-slate-800 dark:bg-slate-950 dark:focus:border-indigo-500/40 dark:focus:ring-indigo-500/20"
                                onChange={(e) => {
                                  const nextOverrides = { ...currentOverrides }
                              ...
                              <p className="text-muted-foreground text-xs">
                                Choose a dedicated text channel for this event,
                                or leave it on the linked default.
                              </p>
                            </div>
                          </div>
                        )
